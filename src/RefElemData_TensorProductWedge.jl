...
# Jesse's implementation of RefElemData for wedges
function MyRefElemData(elem::Wedge, approximation_type::TensorProductWedge; kwargs...)
...
function RefElemData(elem::Wedge, approximation_type::TensorProductWedge; kwargs...)
    ...
    quad_face_ids(f) = (1:num_quad_face_nodes) .+ (f - 1) * num_quad_face_nodes
    tri_face_ids(f) = (1:num_tri_face_nodes) .+ (f - 1) * num_tri_face_nodes .+
                      3 * num_quad_face_nodes
    node_ids_by_face = (quad_face_ids(1), quad_face_ids(2), quad_face_ids(3),
                        tri_face_ids(1), tri_face_ids(2))
    ...
    _, sp = _wedge_tensor_product(line.rp, tri.sp)
    # `line.Vp` is a `UniformScaling` type for `RefElemData` from SummationByPartsOperators.jl
    Vp = line.Vp isa UniformScaling ? kron(I(num_line_nodes), tri.Vp) :
         kron(line.Vp, tri.Vp)
...
function inverse_trace_constant(rd::RefElemData{3, <:Wedge, <:TensorProductWedge})
    inverse_trace_constant(rd.approximation_type.line) +
    inverse_trace_constant(rd.approximation_type.tri)
end
